...
pipe = AutoPipelineForInpainting.from_pretrained(
    ...
    # use_safetensors=True # note SD is in diffusers format.
).to(DEVICE)
...
######
def log_to_csv(filename, view, active_pathologies):
    """
    ...
    # Create a row.
    row = [global_counter, filename, view]
    ...
    for mask_area in MASK_AREAS: # joint, upper, whole.
        ...
        for pathology in PATHOLOGIES_LIST:
            ...
            for i in range(500):
                ...
                log_to_csv(save_name, view, [pathology])
...
for pathology in PATHOLOGIES_LIST:
    ...
    for view in VIEWS:
        ...
        for base_name in files:
            ...
            genImg(prompt, base_image, mask_image, dest_folder, save_name)
            log_to_csv(save_name, view, []) # See above, empty list => normal image.
...
for num_pathos in counts_to_generate:
    ...
    for view in VIEWS:
        ...
        for i in range(500):
            ...
            genImg(prompt, base_image, mask_image, dest_folder, save_name)
            log_to_csv(save_name, view, selected_pathos)
